...
char* readBinary(const char* fileName, size_t* givenFileSize) {
	...
	if (fileHandler) {
		...
		fileSize = ftell(fileHandler); //getting the size
	...
	*givenFileSize = fileSize;
	//so binaryContent is form beginning to the end of executable
...
char* createNewSectionHeader(char* imageBase, unsigned char* packedContent, size_t packedLength, size_t* newFileSize) {
	...
	IMAGE_NT_HEADERS* ntHeaderOfImage = ntHeaders(imageBase); //getting the header of the loader
	...
	if (checkBoundary) {
		...
		//packedLength will be the size of our binary we want to encrypt
		if (packedLength % 16) {
			//packedContent is the binaryContent
		...
		//encrypting binaryContent after padding it 
